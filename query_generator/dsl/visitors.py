...
class VisitorNamingCheck(Visitor):
...
class VisitorSemanticCheck(Visitor):
    def __init__(self, symbol_table, join_graph) -> None:
                ...
                if isinstance(attr, Attribute):
                    ...
                    if attr.foreign_key:
                        if len(attr_expr.elements) > 1:
                            dsl_logger.error(f"Foreign Keys dont be declared in composite attributes: attribute number {index} in dimensional table {dimensional_table.name}")
                            self.good_semantic = False
                        dimension = attr.foreign_key[0]
                        if dimension not in self.symbol_table.keys():
                            ...
                            if attr.foreign_key[1] not in self.symbol_table[dimension]:
                                dsl_logger.error(f'Attribute {attr.foreign_key[1]} not declared in dimensional table {dimension}: attribute number {index} in dimensional table {dimensional_table.name}')
                                self.good_semantic = False        
                ...
                if isinstance(attr, (AggAttribute, Attribute, AttributeFunction)):
                    ...
                    if attr.table_name != 'self': 
                        if attr.table_name not in self.join_graph.nodes.keys():
                            dsl_logger.error(f'Source table {attr.table_name} not exist in the source: attribute number {index} in dimensional table {dimensional_table.name}')
                            self.good_semantic = False
                
                        else: 
                            find_attr = False
                            for attr_name, _, _ in self.join_graph.nodes[attr.table_name]['attrs']:
                                if attr.name == attr_name:
                                    find_attr = True
                                    break

                            if not find_attr:
                                dsl_logger.error(f'Attribute {attr.name} not defined in source table {attr.table_name}: attribute number {index} in dimensional table {dimensional_table.name}')
                                self.good_semantic = False
                    else:
                        if isinstance(attr, Attribute):
                            if not attr.primary_key:
                                dsl_logger.error(f"Reserved word self its only for autogenerated PKs: attribute number {index} in dimensional table {dimensional_table.name}")
                                self.good_semantic = False
                            else:
                                if not isinstance(dimensional_table, Fact):
                                    dsl_logger.error(f"Reserved word self its only for autogenerated PKs in Facts tables: attribute number {index} in dimensional table {dimensional_table.name}")
                                    self.good_semantic = False
        ...
        for attr_expr, index in zip(dimensional_table.list_attr, range(1, len(dimensional_table.list_attr)+1)):
            if len(attr_expr.elements) > 1:
                ...
                if attr_expr.exp_type:
                    ...
                    if attr.table_name == 'self':
                        dsl_logger.error(f"The serial type must be specified for auto-generated PKs: attribute number {index} in dimensional table {dimensional_table.name}")
                        self.good_type = False
